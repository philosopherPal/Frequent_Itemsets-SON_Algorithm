...
object Problem2 {
  def main(args: Array[String]) {
    ...
    var mapPhase1 = baskets.mapPartitions( lists => Apriori(lists, minSupportSize / chunkSize))//.sortByKey()
    ...
    val pwrite = new PrintWriter(new File("Pallavi_Taneja_SON_Beauty.case" + args(0).toString + "-" + args(2).toString + ".txt" ))
    for (line <- result) {
      pwrite.write(line + "\n")
    }
    pwrite.close()
